...
export class ConfigManager {
  ...
  private loadConfig(): BotConfig {
    ...
    const envConfig: Partial<BotConfig> = {
      server: {
        ...
        port: parseInt(process.env.MC_PORT || String(defaultConfig.server.port), 10),
        version: process.env.MC_VERSION || defaultConfig.server.version,
      },
      bot: {
        username: process.env.MC_USERNAME || defaultConfig.bot.username,
        auth: (process.env.MC_AUTH as 'offline' | 'microsoft') || defaultConfig.bot.auth,
        maxRetries: parseInt(process.env.MC_MAX_RETRIES || String(defaultConfig.bot.maxRetries), 10),
        retryDelay: parseInt(process.env.MC_RETRY_DELAY || String(defaultConfig.bot.retryDelay), 10),
      },
      features: {
        autoEat: process.env.MC_AUTO_EAT === 'true' || defaultConfig.features.autoEat,
        autoRespawn: process.env.MC_AUTO_RESPAWN === 'true' || defaultConfig.features.autoRespawn,
        allowCommands: process.env.MC_ALLOW_COMMANDS?.split(',') || defaultConfig.features.allowCommands,
        enableLogging: process.env.MC_ENABLE_LOGGING !== 'false',
        logLevel: (process.env.MC_LOG_LEVEL as any) || defaultConfig.features.logLevel,
      },
    };
    ...
    for (const obj of objects) {
      for (const key in obj) {
        if (obj[key] && typeof obj[key] === 'object' && !Array.isArray(obj[key])) {
          (result as any)[key] = this.deepMerge((result as any)[key] || {}, obj[key]);
        } else {
  ...
  public validateConfig(): { isValid: boolean; errors: string[] } {
    ...
    // 機能設定の検証
    if (!['debug', 'info', 'warn', 'error'].includes(this.config.features.logLevel)) {
      errors.push('Log level must be one of: debug, info, warn, error');
    }

    // アビリティ設定の検証
    if (this.config.abilities.vitals.healthThreshold < 0 || this.config.abilities.vitals.healthThreshold > 20) {
      errors.push('Health threshold must be between 0 and 20');
    }

    if (this.config.abilities.vitals.hungerThreshold < 0 || this.config.abilities.vitals.hungerThreshold > 20) {
