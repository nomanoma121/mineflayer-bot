...
export class Logger {
  ...
  public static bot = {
    ...
    commandExecuted: (command: string, username: string, botName: string, success: boolean) => {
    ...
    // アビリティ関連
    abilityUsed: (ability: string, method: string, botName: string, success: boolean) => {
    ...
    abilityError: (ability: string, method: string, error: Error, botName: string) => {
    ...
    // 環境関連
    entityDetected: (entityType: string, entityName: string, distance: number, botName: string) => {
    ...
    // インベントリ関連
    itemUsed: (item: string, count: number, action: string, botName: string) => {
  ...
  public static setLevel(level: string): void {
    if (this.initialized) {
      ...
      this.instance.transports.forEach(transport => {
  ...
  public static getStats(): {
    ...
    return {
      level: this.instance?.level || 'unknown',
      transports: this.instance?.transports?.length || 0,
