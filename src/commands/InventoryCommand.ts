...
export class InventoryCommand implements ICommand {
  ...
  public async execute(bot: Bot, username: string, args: string[]): Promise<void> {
    try {
      ...
      inventoryLines.push(`合計: ${totalItems}個 (${items.length}/${bot.mc.inventory.slots.length - 5}スロット使用)`);
      ...
      bot.sendMessage(`インベントリ表示中にエラーが発生しました: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
