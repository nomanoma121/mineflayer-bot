...
export class SetRespawnCommand implements ICommand {
  ...
  public async execute(bot: Bot, username: string, args: string[]): Promise<void> {
    try {
      console.log(`[${bot.getName()}] SetRespawn command executed by ${username}`);
      
      // 周辺のベッドを検索
      const beds = this.findNearbyBeds(bot);
      
      if (beds.length === 0) {
        bot.sendMessage('近くにベッドが見つかりません。ベッドに近づいてください。');
        return;
      }
      ...
      if (distance > 4) {
        bot.sendMessage(`ベッドまでの距離が遠すぎます（距離: ${distance.toFixed(1)}）。近づいてください。`);
        return;
      }
      
      bot.sendMessage(`ベッドを使用してリスポーン地点を設定します... (${closestBed.position.x}, ${closestBed.position.y}, ${closestBed.position.z})`);
      ...
      try {
        ...
        if (error instanceof Error) {
          if (error.message.includes('cannot sleep')) {
            bot.sendMessage('今は寝ることができません。敵が近くにいるか、時間が適切ではありません。');
          } else if (error.message.includes('bed is occupied')) {
            bot.sendMessage('このベッドは既に使用されています。');
          } else {
            bot.sendMessage(`ベッドの使用中にエラーが発生しました: ${error.message}`);
          }
        } else {
      ...
      bot.sendMessage(`リスポーン地点設定中にエラーが発生しました: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
