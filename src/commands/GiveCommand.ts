...
export class GiveCommand implements ICommand {
	...
	public async execute(
		...
		try {
			...
			if (args.length < 2) {
				bot.sendMessage(
					"使用法: @botname give <プレイヤー名> <アイテム名> [数量]",
				);
				return;
			}
			...
			// プレイヤーまでの距離をチェック
			const distance = bot.mc.entity.position.distanceTo(
				targetPlayer.entity.position,
			);
			if (distance > 4) {
				bot.sendMessage(
					`プレイヤー「${targetPlayerName}」までの距離が遠すぎます（距離: ${distance.toFixed(1)}）。近づいてください。`,
				);
				return;
			}
			...
			bot.sendMessage(
				`${targetPlayerName} に ${firstItem.displayName} x${quantity} を渡します...`,
			);
			...
			for (const item of matchingItems) {
				...
				try {
					...
					console.log(
						`[${bot.getName()}] Tossed ${toToss} ${item.displayName} towards ${targetPlayerName}`,
					);
				} catch (error) {
					console.error(`Error tossing item:`, error);
					bot.sendMessage(
						`アイテムの投擲中にエラーが発生しました: ${error instanceof Error ? error.message : "Unknown error"}`,
					);
					return;
				}
			}
			...
			bot.sendMessage(
				`${targetPlayerName} に ${firstItem.displayName} x${quantity} を渡しました！`,
			);
		} catch (error) {
			console.error(`[${bot.getName()}] Error in give command:`, error);
			bot.sendMessage(
				`アイテム提供中にエラーが発生しました: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
