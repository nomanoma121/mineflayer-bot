...
export class AttackCommand implements ICommand {
	...
	public async execute(
		...
		try {
			...
			// 近くのエンティティを検索
			const nearbyEntities = EntityUtils.findNearbyEntitiesByName(
				bot,
				targetName,
				20,
			);
			...
			bot.sendMessage(
				`${targetDisplayName} を攻撃します！（距離: ${distance.toFixed(1)}）`,
			);

			// 攻撃状態に変更（現在の状態を保存して戦闘後に復帰）
			const currentState = bot.getCurrentState();
			const attackingState = new AttackingState(
				bot,
				target,
