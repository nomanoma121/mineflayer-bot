...
export class ServantCommand implements ICommand {
  ...
  public async execute(bot: Bot, username: string, args: string[]): Promise<void> {
    try {
      ...
      console.log(`[${bot.getName()}] Servant command executed by ${username}, master: ${masterName}`);
      
      // マスターとなるプレイヤーがサーバーに存在するかチェック
      const master = bot.mc.players[masterName];
      if (!master) {
        bot.sendMessage(`エラー: プレイヤー '${masterName}' が見つかりません。`);
        console.log(`[${bot.getName()}] Master player '${masterName}' not found`);
        return;
      }
      
      // マスターが実際にワールドに存在するかチェック
      if (!master.entity) {
        bot.sendMessage(`エラー: プレイヤー '${masterName}' はワールドに存在しません。`);
        console.log(`[${bot.getName()}] Master player '${masterName}' entity not found`);
        return;
      }
      ...
      if (masterName === username) {
        bot.sendMessage(`${username}様のサーバントになりました。お守りいたします。`);
      } else {
        bot.sendMessage(`${username}様の指示により、${masterName}様のサーバントになりました。`);
      }
