...
export class AbilityTestCommand implements ICommand {
  ...
  public async execute(
    ...
    try {
      ...
      if (args.length === 0) {
        bot.say.say("使用方法: @bot abilitytest <vitals|sensing|inventory|say|status|emergency>");
        return;
      }
      ...
      switch (testType) {
        ...
        case "emergency":
          ...
          break;
        default:
          bot.say.say("不明なテストタイプです。vitals, sensing, inventory, say, status, emergency のいずれかを指定してください。");
          break;
      }
  ...
  private async testSensing(bot: Bot): Promise<void> {
    ...
    bot.say.say(`現在位置: X=${Math.floor(pos.x)}, Y=${Math.floor(pos.y)}, Z=${Math.floor(pos.z)}`);
  ...
  private async testInventory(bot: Bot): Promise<void> {
    ...
    bot.say.say(`インベントリ: ${invInfo.usedSlots}/${invInfo.totalSlots} スロット使用中`);
  ...
  private async testStatus(bot: Bot): Promise<void> {
    ...
    bot.say.say(`利用可能なアビリティ数: ${bot.getAbilityManager().getAvailableAbilities().length}`);
