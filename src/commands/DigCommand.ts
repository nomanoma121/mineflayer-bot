    ...
    try {
      ...
      if (args.length === 3) {
        ...
        if (isNaN(x) || isNaN(y) || isNaN(z)) {
          bot.sendMessage('座標は数値で指定してください。使用法: @botname dig <x> <y> <z>');
          return;
        }
        ...
        bot.sendMessage('使用法: @botname dig [x y z] （座標指定なしの場合は見ているブロックを掘ります）');
        return;
      }
      ...
      if (distance > 6) {
        bot.sendMessage(`ブロックまでの距離が遠すぎます（距離: ${distance.toFixed(1)}）。近づいてください。`);
        return;
      }
      
      bot.sendMessage(`${targetBlock.name} を掘り始めます... (${targetBlock.position.x}, ${targetBlock.position.y}, ${targetBlock.position.z})`);
      ...
      bot.sendMessage(`掘削中にエラーが発生しました: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
