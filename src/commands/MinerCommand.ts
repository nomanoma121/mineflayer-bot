...
export class MinerCommand implements ICommand {
  ...
  public async execute(bot: Bot, username: string, args: string[]): Promise<void> {
    try {
      ...
      // Y座標の安全性をチェック
      if (Math.min(y1, y2) < 5) {
        bot.sendMessage("警告: Y座標が5未満です。岩盤に近いため注意してください。");
      }
      if (Math.max(y1, y2) > 100) {
        bot.sendMessage("警告: Y座標が100を超えています。地上での採掘になります。");
      }
      ...
      if (volume > 50000) { // 50x50x20ブロック以上の場合は警告
        bot.sendMessage("警告: 採掘エリアが大きすぎます。処理に非常に時間がかかる可能性があります。");
      }
      
      console.log(`[${bot.getName()}] Miner command executed by ${username}`);
      console.log(`[${bot.getName()}] Mining area: (${x1},${y1},${z1}) to (${x2},${y2},${z2}), Volume: ${volume} blocks`);
      ...
      // 採掘状態に遷移
      bot.changeState(new MiningState(bot, corner1, corner2));
      
      // 成功メッセージを送信
      bot.sendMessage(`${username}様の指示により、指定された範囲での採掘作業を開始します。`);
      bot.sendMessage(`採掘エリア: (${x1},${y1},${z1}) から (${x2},${y2},${z2})`);
