...
export class SetHomeCommand implements ICommand {
  ...
  public async execute(bot: Bot, username: string, args: string[]): Promise<void> {
    try {
      ...
      console.log(`[${bot.getName()}] Home position set to: (${homePosition.x}, ${homePosition.y}, ${homePosition.z})`);

    } catch (error) {
      console.error(`[${bot.getName()}] Error in sethome command:`, error);
      bot.sendMessage(`拠点設定中にエラーが発生しました: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
