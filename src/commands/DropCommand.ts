...
export class DropCommand implements ICommand {
  ...
  public async execute(bot: Bot, username: string, args: string[]): Promise<void> {
    try {
      ...
      bot.sendMessage(`${firstItem.displayName} x${quantity} をドロップします...`);
      ...
      for (const item of matchingItems) {
        ...
        try {
          ...
          console.log(`[${bot.getName()}] Dropped ${toDrop} ${item.displayName}`);
        } catch (error) {
          console.error(`Error dropping item:`, error);
          bot.sendMessage(`アイテムのドロップ中にエラーが発生しました: ${error instanceof Error ? error.message : 'Unknown error'}`);
          return;
        }
      }
      
      bot.sendMessage(`${firstItem.displayName} x${quantity} をドロップしました！`);
      
    } catch (error) {
      console.error(`[${bot.getName()}] Error in drop command:`, error);
      bot.sendMessage(`アイテムドロップ中にエラーが発生しました: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async dropAllItems(bot: Bot): Promise<void> {
    ...
    for (const item of items) {
      try {
        ...
        console.log(`[${bot.getName()}] Dropped all ${item.displayName} (${item.count})`);
      } catch (error) {
        console.error(`Error dropping ${item.displayName}:`, error);
        bot.sendMessage(`${item.displayName} のドロップ中にエラーが発生しました。`);
      }
    }
