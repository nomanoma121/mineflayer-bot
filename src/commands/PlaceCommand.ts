    ...
    try {
      ...
      if (args.length === 4) {
        ...
        if (isNaN(x) || isNaN(y) || isNaN(z)) {
          bot.sendMessage('座標は数値で指定してください。使用法: @botname place <アイテム名> <x> <y> <z>');
          return;
        }
        ...
        if (!targetBlock) {
          bot.sendMessage('設置する場所が見つかりません。ブロックを見つめてください。');
          return;
        }
      ...
      // アイテムをインベントリから検索
      const item = bot.mc.inventory.items().find(
      ...
      if (distance > 6) {
        bot.sendMessage(`設置位置までの距離が遠すぎます（距離: ${distance.toFixed(1)}）。近づいてください。`);
        return;
      }
      ...
      if (blockAtPosition && blockAtPosition.name !== 'air') {
        bot.sendMessage(`設置位置 (${targetPosition.x}, ${targetPosition.y}, ${targetPosition.z}) には既にブロックがあります。`);
        return;
      }
      ...
      bot.sendMessage(`${item.displayName} を設置します... (${targetPosition.x}, ${targetPosition.y}, ${targetPosition.z})`);
      ...
      bot.sendMessage(`設置中にエラーが発生しました: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
