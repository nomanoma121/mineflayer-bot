		...
		try {
			...
			if (args.length > 0) {
				...
				if (validTreeTypes.includes(args[0])) {
					...
					bot.sendMessage(
						`エラー: 有効な木の種類を指定してください (${validTreeTypes.join(", ")})`,
					);
					return;
				}
			}
			...
			console.log(
				`[${bot.getName()}] Lumberjack command executed by ${username}, tree type: ${treeType}, range: ${range}`,
			);
			...
			console.log(
				`[${bot.getName()}] Changing state from ${currentStateName} to Lumberjacking`,
			);

			// 木こり状態に遷移
			await bot.changeState(new LumberjackingState(bot, treeType, range));

			// 成功メッセージを送信
			bot.sendMessage(
				`${username}様の指示により、${treeType}の木こり作業を開始します。範囲: ${range}ブロック`,
			);
		} catch (error) {
