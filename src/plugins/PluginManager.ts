...
export class PluginManager {
  ...
  constructor(pluginsDirectory?: string) {
    this.pluginsDirectory = pluginsDirectory || path.join(process.cwd(), 'plugins');
  }
  ...
  public registerPlugin(plugin: IPlugin): void {
    ...
    if (this.plugins.has(metadata.name)) {
      Logger.structured.warn(`Plugin ${metadata.name} is already registered, overwriting`);
    }
    
    this.plugins.set(metadata.name, plugin);
    Logger.structured.info(`Plugin registered: ${metadata.name} v${metadata.version}`);
  }
  ...
  public async loadPluginsFromDirectory(): Promise<void> {
    if (!fs.existsSync(this.pluginsDirectory)) {
      Logger.structured.info(`Plugins directory does not exist: ${this.pluginsDirectory}`);
      return;
    }
    ...
    for (const file of pluginFiles) {
      try {
        ...
        Logger.structured.error(`Failed to load plugin from ${file}`, error as Error);
      }
    }
  }
  ...
  public async initializePlugin(pluginName: string): Promise<void> {
    ...
    try {
      ...
      if (metadata.dependencies) {
        for (const dep of metadata.dependencies) {
          if (!this.plugins.has(dep) || !this.enabledPlugins.has(dep)) {
            throw new Error(`Dependency ${dep} not available for plugin ${pluginName}`);
          }
        }
      }
      ...
      Logger.structured.error(`Failed to initialize plugin ${pluginName}`, error as Error);
      throw error;
    }
  }
  ...
  public async enablePlugin(pluginName: string): Promise<void> {
    ...
    try {
      ...
      Logger.structured.error(`Failed to enable plugin ${pluginName}`, error as Error);
      throw error;
    }
  }
  ...
  public async disablePlugin(pluginName: string): Promise<void> {
    ...
    try {
      ...
      Logger.structured.error(`Failed to disable plugin ${pluginName}`, error as Error);
      throw error;
    }
  }
  ...
  public async enableAllPlugins(): Promise<void> {
    ...
    for (const pluginName of sortedPlugins) {
      try {
        ...
        Logger.structured.error(`Failed to enable plugin ${pluginName}, skipping`, error as Error);
      }
    }
  }
