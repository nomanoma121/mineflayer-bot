...
describe("SayAbility", () => {
  ...
  describe("initialization", () => {
    ...
    it("should have correct description", () => {
      expect(sayAbility.getDescription()).toBe("チャットでの発言やメッセージ送信を管理します");
    });
  ...
  describe("structured messaging", () => {
    it("should send status report", () => {
      ...
      expect(bot.sendMessage).toHaveBeenCalledWith("[状況報告] Operating normally");
    });

    it("should send error report", () => {
      sayAbility.reportError("Something went wrong");
      expect(bot.sendMessage).toHaveBeenCalledWith("[エラー] Something went wrong");
    });

    it("should send success report", () => {
      sayAbility.reportSuccess("Task completed");
      expect(bot.sendMessage).toHaveBeenCalledWith("[成功] Task completedが完了しました");
    });
  ...
  describe("random messages", () => {
    it("should send random encouragement", () => {
      ...
      const encouragements = [
        "頑張りましょう！", "一緒にがんばろう！", "きっとできる！",
        "諦めないで！", "応援しています！", "ファイト！", "その調子！", "素晴らしい！"
      ];
    ...
    it("should send random greeting", () => {
      ...
      const greetings = [
        ...
        "よろしくお願いします！", "元気ですか？", "調子はどうですか？", "今日もよろしく！"
      ];
  ...
  describe("whisper functionality", () => {
    it("should send whisper message", () => {
      ...
      expect(bot.sendMessage).toHaveBeenCalledWith("/msg target_player secret message");
    });
  ...
  describe("status reporting shortcuts", () => {
    ...
    it("should report position", () => {
      ...
      expect(bot.sendMessage).toHaveBeenCalledWith("現在位置: X=100, Y=64, Z=-200");
    });

    it("should report health", () => {
      sayAbility.reportHealth();
      expect(bot.sendMessage).toHaveBeenCalledWith("体力: 15/20, 空腹度: 18/20");
    });

    it("should report inventory", () => {
      sayAbility.reportInventory();
      expect(bot.sendMessage).toHaveBeenCalledWith("インベントリ: 2個のアイテム, 空きスロット: 5");
    });
