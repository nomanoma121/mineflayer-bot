...
describe("AbilityManager", () => {
  ...
  describe("system status", () => {
    ...
    it("should indicate unavailable abilities in status", () => {
      ...
      const unavailableAbility = status.abilitiesList.find(a => a.name === "UnavailableAbility");
  ...
  describe("diagnostics", () => {
    ...
    it("should report critical status with many issues", () => {
      ...
      jest.spyOn(abilityManager.inventory, 'isAvailable').mockReturnValue(false);
    ...
    it("should provide specific recommendations", () => {
      ...
      expect(diagnosis.recommendations).toContain("initialize(bot)メソッドを呼び出してください");
    });
  });
