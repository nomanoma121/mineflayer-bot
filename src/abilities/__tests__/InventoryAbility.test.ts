...
describe("InventoryAbility", () => {
	...
	describe("initialization", () => {
		...
		it("should have correct description", () => {
			expect(inventoryAbility.getDescription()).toBe(
				"インベントリとアイテムの管理を行います",
			);
		});
	...
	describe("food detection", () => {
		...
		it("should return null when no food available", () => {
			mockMinecraftBot.inventory.items = jest.fn(
				() => [{ name: "stone", count: 10 } as Item] as Item[],
			);
	...
	describe("weapon detection", () => {
		...
		it("should find weapons", () => {
			...
			expect(["wooden_sword", "iron_axe", "diamond_sword"]).toContain(
				weapon?.name,
			);
		});
		...
		it("should prefer higher damage weapons", () => {
			...
			expect(["diamond_sword", "iron_axe", "wooden_sword"]).toContain(
				weapon?.name,
			);
		});
		...
		it("should return null when no weapons available", () => {
			mockMinecraftBot.inventory.items = jest.fn(
				() => [{ name: "bread", count: 1 } as Item] as Item[],
			);
	...
	describe("equipment operations", () => {
		...
		it("should handle equip errors", async () => {
			const sword = { name: "iron_sword" } as Item;
			mockMinecraftBot.equip = jest
				.fn()
				.mockRejectedValue(new Error("Cannot equip"));

			await expect(inventoryAbility.equip(sword)).rejects.toThrow(
				"Cannot equip",
			);
		});
	});
	...
	describe("item dropping", () => {
		...
		it("should throw error for non-existent items", async () => {
			await expect(inventoryAbility.dropItem("nonexistent")).rejects.toThrow(
				"アイテム「nonexistent」が見つかりません",
			);
		});
	});
	...
	describe("chest operations", () => {
		...
		it("should handle missing items in chest", async () => {
			...
			await expect(
				inventoryAbility.withdraw(emptyChest, "bread"),
