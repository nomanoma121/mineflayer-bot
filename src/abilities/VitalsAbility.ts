  ...
  findSafeSpot(radius: number = 10): Vec3 | null {
    ...
    for (let x = -radius; x <= radius; x += 2) {
      for (let z = -radius; z <= radius; z += 2) {
        ...
        // 地面の高さを見つける
        let groundY = Math.floor(checkPos.y);
        for (let y = groundY; y >= groundY - 5; y--) {
          const blockBelow = this.bot.mc.blockAt(checkPos.floored().offset(0, y - 1, 0));
          const blockAt = this.bot.mc.blockAt(checkPos.floored().offset(0, y, 0));
          const blockAbove = this.bot.mc.blockAt(checkPos.floored().offset(0, y + 1, 0));
          
          // 地面に固体ブロックがあり、その上2ブロックが空気の場合は安全
          if (blockBelow && blockAt && blockAbove) {
            if (blockBelow.type !== 0 && blockAt.type === 0 && blockAbove.type === 0) {
