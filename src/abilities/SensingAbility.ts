...
export class SensingAbility implements IAbility {
  ...
  canSeePosition(position: Vec3): boolean {
    ...
    // レイキャストで障害物をチェック
    const raycast = this.bot.mc.world.raycast(from, to.minus(from).normalize(), to.distanceTo(from));
  ...
  findNearestHostileMob(): Entity | null {
    const hostileMobs = [
      ...
      "phantom", "pillager", "vindicator", "evoker", "ravager", "warden"
    ];
  ...
  findNearestAnimal(): Entity | null {
    const animals = [
      ...
      "llama", "cat", "dog", "wolf", "ocelot", "parrot", "turtle"
    ];
   ...
   */
  countNearbyEntities(filter: (e: Entity) => boolean, radius: number = 16): number {
  ...
  getEnvironmentInfo(): {
    ...
    return {
      ...
      weather: this.getWeatherInfo(),
      time: this.getTimeInfo(),
      nearbyPlayersCount: this.countNearbyEntities((e: Entity) => e.type === "player"),
