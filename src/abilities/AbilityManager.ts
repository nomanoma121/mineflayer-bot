...
export class AbilityManager {
  ...
  getAvailableAbilities(): IAbility[] {
    return Array.from(this.abilities.values()).filter(ability => ability.isAvailable());
  }
  ...
  diagnose(): {
    ...
    for (const ability of this.abilities.values()) {
      if (!ability.isAvailable()) {
        ...
        recommendations.push(`${ability.getName()}アビリティの状態を確認してください`);
      }
    }
  ...
  getStats(): {
    ...
    return {
      vitals: vitalsAbility?.isAvailable() ? vitalsAbility.getVitalStats() : null,
      environment: sensingAbility?.isAvailable() ? sensingAbility.getEnvironmentInfo() : null,
      inventory: inventoryAbility?.isAvailable() ? inventoryAbility.getInventoryInfo() : null,
      messageHistory: sayAbility?.isAvailable() ? sayAbility.getMessageHistory(5) : null,
    };
  }
    ...
    try {
      ...
      if (vitalsAbility.isInDanger()) {
        ...
        if (safeSpot) {
          ...
          sayAbility.reportInfo(`安全な場所を発見: ${safeSpot.x}, ${safeSpot.y}, ${safeSpot.z}`);
        }
