...
export class FarmingState implements IBotState {
  ...
  private currentTask: "scanning" | "moving" | "harvesting" | "planting" = "scanning";
  ...
  private scanForMatureCrops(): void {
    ...
    if (matureCrop) {
      console.log(`[${this.bot.getName()}] Found mature crop: ${matureCrop.name} at (${matureCrop.position.x}, ${matureCrop.position.y}, ${matureCrop.position.z})`);
  ...
  private async harvestCrop(): Promise<void> {
    ...
    try {
      ...
      console.log(`[${this.bot.getName()}] Harvested ${this.currentTarget.name}`);
  ...
  private async replantCrop(): Promise<void> {
    ...
    try {
      ...
      const seed = this.bot.mc.inventory.items().find(item => item.name === seedName);
      
      if (seed) {
        // 土の上に種を植える
        const soilBlock = this.bot.mc.blockAt(this.currentTarget.position.offset(0, -1, 0));
        if (soilBlock && (soilBlock.name === "farmland" || soilBlock.name === "dirt")) {
        ...
        console.log(`[${this.bot.getName()}] No seeds available for replanting`);
      }
    } catch (error) {
