...
export class ExploringState implements IBotState {
	...
	public execute(): void {
		...
		if (this.hasReachedTarget()) {
			console.log(
				`[${this.bot.getName()}] Reached exploration target, marking area as explored`,
			);
	...
	private setNextExplorationTarget(): void {
		...
		if (target) {
			...
			console.log(
				`[${this.bot.getName()}] New exploration target: (${target.x.toFixed(1)}, ${target.y.toFixed(1)}, ${target.z.toFixed(1)})`,
			);
			this.moveToTarget();
		} else {
			// 探索範囲を拡大
			this.explorationRadius = Math.min(
				this.explorationRadius + 50,
				this.maxRadius,
			);
			console.log(
				`[${this.bot.getName()}] Expanding exploration radius to ${this.explorationRadius}`,
			);

			if (this.explorationRadius >= this.maxRadius) {
				console.log(
					`[${this.bot.getName()}] Reached maximum exploration radius, stopping`,
				);
	...
	private moveToTarget(): void {
		...
		try {
			...
			console.log(
				`[${this.bot.getName()}] Starting movement to exploration target`,
			);
		} catch (error) {
			console.error(
				`[${this.bot.getName()}] Error setting exploration target:`,
				error,
			);
			// エラーが発生した場合は新しい目標を設定
			this.setNextExplorationTarget();
	...
	private markCurrentChunkAsExplored(): void {
		...
		console.log(
			`[${this.bot.getName()}] Marked chunk as explored: ${chunkKey}`,
		);
	}
