...
export class MovingState implements IBotState {
	...
	public async enter(): Promise<void> {
		console.log(
			`[${this.bot.getName()}] Entering Moving State - Target: (${this.targetPosition.x}, ${this.targetPosition.y}, ${this.targetPosition.z})`,
		);
		this.isActive = true;

		this.bot.sendMessage(
			`座標 (${Math.floor(this.targetPosition.x)}, ${Math.floor(this.targetPosition.y)}, ${Math.floor(this.targetPosition.z)}) への移動を開始します。`,
		);
		this.startMoving();
	}
	...
	private startMoving(): void {
		try {
			...
			console.log(
				`[${this.bot.getName()}] Moving to (${this.targetPosition.x}, ${this.targetPosition.y}, ${this.targetPosition.z})`,
			);
		} catch (error) {
			console.error(`[${this.bot.getName()}] Error starting movement:`, error);
			this.bot.sendMessage(
				`移動開始中にエラーが発生しました: ${error instanceof Error ? error.message : "Unknown error"}`,
			);

			if (this.onError) {
				this.onError(
					error instanceof Error ? error : new Error("Unknown movement error"),
				);
			}
		}
	}
