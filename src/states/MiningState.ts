...
export class MiningState implements IBotState {
	...
	private currentTask: "moving" | "mining" | "depositing" | "placing_torch" =
		"moving";
	...
	public async enter(): Promise<void> {
		...
		console.log(
			`[${this.bot.getName()}] Mining area: (${this.miningArea.min.x},${this.miningArea.min.y},${this.miningArea.min.z}) to (${this.miningArea.max.x},${this.miningArea.max.y},${this.miningArea.max.z})`,
		);
	...
	public getRecommendedEquipment(): any | null {
		...
		for (const priorityTool of toolPriority) {
			const tool = tools.find((t) =>
				t.name.toLowerCase().includes(priorityTool),
			);
			if (tool) {
				return tool;
			}
	...
	private generateMiningPattern(): void {
		...
		console.log(
			`[${this.bot.getName()}] Generated mining pattern with ${this.miningPattern.length} blocks`,
		);
	}
	...
	private setNextMiningTarget(): void {
		...
		console.log(
			`[${this.bot.getName()}] Next mining target: (${this.currentTarget.x}, ${this.currentTarget.y}, ${this.currentTarget.z})`,
		);
		this.moveToTarget();
	}
	...
	private moveToTarget(): void {
		...
		try {
			...
			console.error(
				`[${this.bot.getName()}] Error moving to mining target:`,
				error,
			);
			this.setNextMiningTarget();
		}
	...
	private checkMovementProgress(): void {
		...
		if (distance <= 3 || !this.bot.mc.pathfinder.isMoving()) {
			console.log(
				`[${this.bot.getName()}] Reached mining target, starting to mine`,
			);
	...
	private async placeTorch(): Promise<void> {
		try {
			const torch = this.bot.mc.inventory
				.items()
	...
	private dropValuableItems(): void {
		...
		for (const item of this.bot.mc.inventory.items()) {
			if (valuableItems.includes(item.name)) {
				...
				console.log(
					`[${this.bot.getName()}] Dropped ${item.count} ${item.name}`,
				);
			}
		}
	}
