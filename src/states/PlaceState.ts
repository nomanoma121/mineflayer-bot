...
export class PlaceState implements IBotState {
  ...
  public async enter(): Promise<void> {
    console.log(`[${this.bot.getName()}] Entering Place State for ${this.blockName} at ${this.targetPosition}`);
    this.isActive = true;
    ...
    this.bot.sendMessage(`${this.blockName}を座標 (${Math.floor(this.targetPosition.x)}, ${Math.floor(this.targetPosition.y)}, ${Math.floor(this.targetPosition.z)}) に設置します。`);
  ...
  private async placeBlock(): Promise<void> {
    try {
      ...
      const item = this.bot.mc.inventory
        ...
        .find(
          (item) =>
            ...
            item.displayName.toLowerCase().includes(this.blockName.toLowerCase())
        );
      if (!item) {
      ...
      // ブロックを設置
      await this.bot.mc.placeBlock(referenceBlock, this.targetPosition.subtract(referenceBlock.position));
