...
export class DigState implements IBotState {
	...
	public async enter(): Promise<void> {
		console.log(
			`[${this.bot.getName()}] Entering Dig State for block at ${this.targetBlock}`,
		);
		...
		if (block) {
			try {
				...
				console.log(
					`[${this.bot.getName()}] Successfully dug block at ${this.targetBlock}`,
				);
			} catch (error) {
			...
			console.warn(
				`[${this.bot.getName()}] No block found at ${this.targetBlock}`,
			);
		}
	}
