...
export class LumberjackingState implements IBotState {
	...
	private currentTask: "searching" | "moving" | "chopping" | "planting" =
		"searching";
	...
	public async enter(): Promise<void> {
		console.log(
			`[${this.bot.getName()}] Entering Lumberjacking state. Tree type: ${this.treeType}, Range: ${this.range}`,
		);
	...
	public execute(): void {
		...
		if (this.isInventoryFull()) {
			console.log(
				`[${this.bot.getName()}] Inventory is full, stopping lumberjacking`,
			);
			this.bot.sendMessage(
				"インベントリが満杯になったため、木こり作業を終了します。",
			);
	...
	private searchForTree(): void {
		...
		if (tree) {
			console.log(
				`[${this.bot.getName()}] Found ${this.treeType} tree at (${tree.position.x}, ${tree.position.y}, ${tree.position.z})`,
			);
			...
			console.log(
				`[${this.bot.getName()}] No ${this.treeType} trees found in range, searching again...`,
			);
			// 少し待ってから再検索
			setTimeout(() => {
	...
	private async chopTree(): Promise<void> {
		...
		try {
			...
			if (nextLogBlock) {
				...
				console.log(
					`[${this.bot.getName()}] Finished chopping tree, preparing to plant sapling`,
				);
	...
	private async plantSapling(): Promise<void> {
		...
		try {
			...
			const sapling = this.bot.mc.inventory
				.items()
				.find((item) => item.name === saplingName);
	...
	private async equipAxe(): Promise<void> {
		...
		for (const axeName of axes) {
			const axe = this.bot.mc.inventory
				.items()
				.find((item) => item.name === axeName);
			if (axe) {
	...
	private findPlantPosition(): Vec3 | null {
		...
		if (
			groundBlock &&
			(groundBlock.name === "dirt" || groundBlock.name === "grass_block")
		) {
