...
export class WanderingState implements IBotState {
	...
	public async enter(): Promise<void> {
		console.log(
			`[${this.bot.getName()}] Entering Wandering state. Range: ${this.range} blocks`,
		);
	...
	public execute(): void {
		...
		if (this.hasReachedTarget()) {
			console.log(
				`[${this.bot.getName()}] Reached target, setting new random destination`,
			);
			this.setRandomTarget();
		}
	...
	private setRandomTarget(): void {
		...
		console.log(
			`[${this.bot.getName()}] New wander target: (${x.toFixed(1)}, ${y.toFixed(1)}, ${z.toFixed(1)})`,
		);
	...
	private moveToTarget(): void {
		...
		try {
			...
			console.error(
				`[${this.bot.getName()}] Error setting wander target:`,
				error,
			);
			// エラーが発生した場合は新しい目標を設定
			this.setRandomTarget();
